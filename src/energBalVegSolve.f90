...
subroutine energBalVegSolve (ISNOW, FI, & ! Formerly TSOLVC
  ...
  ! SESolver%IntervalBounds=(/4.0, 10.0/)
  ! SESolver%InitialGuess = 5 !x=7
  ! SESolver%DDfDDx_max = 4.0
  ! SESolver%errT = 1.0D-1   !Tol
  ! call SESolver%FindRoot(dummyfunc)
  ...
  do I = IL1,IL2 ! loop 125
    ...
    nSolEnergBalVeg = nSolEnergBalVeg + 1.0
    resiEnergBalVeg = resiEnergBalVeg + RESID(I)*RESID(I)
    TSolEnergBalVeg = TSolEnergBalVeg + TZERO(I)
  end do !loop 125
  call cpu_time(EndTime)
  timeEnergBalVeg = timeEnergBalVeg + (EndTime-StartTime)
...
6380 format(2X,9F10.2)
  ...
  function dummyfunc(x) result(fx)
    real(8)  :: x, fx !x is the root to be determined, fx is the value to be optimized.

    nItrEnergBalVeg = nItrEnergBalVeg + 1

    fx = 1000.0*(x-7.0)*(x-2.0)! explicit expression of the scalar equation
